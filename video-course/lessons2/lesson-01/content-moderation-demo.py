...
content_moderator_key = os.getenv("TWAI900CONTENTSAFETY1_KEY")
content_moderator_endpoint = os.getenv("TWAI900CONTENTSAFETY1_ENDPOINT")
...
def moderate_text():
    ...
    screen = client.text_moderation.screen_text(
        text_content_type="text/plain", text_content=text, language="eng"
    )
...
def moderate_image(image_url):
    ...
    evaluation = client.image_moderation.evaluate_for_adult_racy_content_with_http_info(
        image=image_data, raw=True
    )[0]

    # Print results
    print(
        f"Adult content: {evaluation.adult_classification_score} score, {evaluation.is_adult_content}"
    )
    print(
        f"Racy content: {evaluation.racy_classification_score} score, {evaluation.is_racy_content}"
    )

    # Check for text in image (OCR)
    ocr_result = client.image_moderation.ocr(
        language="eng", image=image_data, raw=True
    )[0]
...
def main():
    ...
    sample_image = (
        "https://moderatorsampleimages.blob.core.windows.net/samples/sample.jpg"
    )
