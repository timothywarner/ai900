...
- CV SDK sample: set `.env`, `npm install`, run. Expect console lines like: `This may be ... (0.xx confidence)`.
- REST sample: set `.env`, `npm init -y && npm install request dotenv`, run. Expect pretty‑printed JSON response.
- Bot: set `.env`, `npm install`, `npm start`. Expect Restify server log with localhost URL; echo replies in Emulator.
