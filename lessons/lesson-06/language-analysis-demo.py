...
translator_location = "YOUR_TRANSLATOR_LOCATION"  # e.g., "eastus"
...
def analyze_sentiment(documents):
    ...
    for idx, doc in enumerate(docs):
        ...
        for sentence_idx, sentence in enumerate(doc.sentences):
            ...
            print(f"  - Scores: Positive={sentence.confidence_scores.positive:.2f}, Neutral={sentence.confidence_scores.neutral:.2f}, Negative={sentence.confidence_scores.negative:.2f}")
...
def recognize_entities(documents):
    ...
    for idx, doc in enumerate(docs):
        ...
        for entity in doc.entities:
            print(f"- Entity: {entity.text} (Category: {entity.category}, Subcategory: {entity.subcategory}, Confidence: {entity.confidence_score:.2f})")
...
def detect_language(documents):
    ...
    for idx, doc in enumerate(docs):
        print(f"\nDocument {idx + 1}:")
        print(f"- Text: {documents[idx]}")
        print(f"- Detected language: {doc.primary_language.name} ({doc.primary_language.iso6391_name})")
